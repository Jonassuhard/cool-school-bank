...
// Herse intro animation - remove overlay after animation completes
setTimeout(() => {
  const herseOverlay = document.getElementById('herse-intro');
  if (herseOverlay) {
    herseOverlay.classList.add('herse-done');
    setTimeout(() => herseOverlay.remove(), 600);
  }
}, 4800);
...
async function assignJob(jobId) {
  ...
  for (const job of jobs) {
    ...
    if (select && select.value) {
      assignments.push({ student_id: parseInt(select.value), job_id: job.id });
    }
  }
